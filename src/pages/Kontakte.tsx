...
export function Kontakte() {
  ...
  const fetchLeads = useCallback(async () => {
    ...
    setLeads(rows)
    setIndustries([...new Set(rows.map(r => r.industry ?? '').filter(Boolean))].sort())
    setCountries([...new Set(rows.map(r => (r as any).country ?? '').filter(Boolean))].sort())
    setLoading(false)
  }, [sort])
  ...
  // Client-seitiger Filter (Suche + Dropdown-Filter)
  const filtered = leads.filter(l => {
    const q = query.toLowerCase()
    const matchQ = !q || [l.company_name, l.full_name, l.email, l.website]
      .some(v => v?.toLowerCase().includes(q))
    const matchS = !filterStatus   || l.status === filterStatus
    const matchI = !filterIndustry || l.industry === filterIndustry
    const matchC = !filterCountry  || (l as any).country === filterCountry
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-zinc-400 dark:text-zinc-500 mt-0.5">
            {loading ? '…' : `${filtered.length} von ${leads.length} Kontakten`}
          </p>
        </div>
        <button
          onClick={fetchLeads}
                ...
                >
                  ...
                  <td className="px-4 py-3 whitespace-nowrap">
                    <span className="text-sm text-zinc-600 dark:text-zinc-400">
                      {(lead as any).country
                        ? (lead as any).state
                          ? `${(lead as any).country} / ${(lead as any).state}`
                          : (lead as any).country
                        : '–'}
                    </span>
                  </td>
        ...
        {!loading && filtered.length > 0 && (
          <div className="px-4 py-3 border-t border-zinc-100 dark:border-zinc-800 flex items-center justify-between">
            <p className="text-xs text-zinc-400">
              {filtered.length} Einträge{filtered.length !== leads.length && ` (gefiltert aus ${leads.length})`}
            </p>
          </div>
        )}
      </div>
